const promotionTemplate = () => ({
  intro: "Порядок повышения по рангам.",
  note: "Если требуется много скриншотов, загружайте их на Imgur или аналогичный сервис. * - задание нельзя пропустить с помощью рекомендации",
  cooldowns: [
    "Мидзуното, Мидзуноэ, Каното, Каноэ — 1 день",
    "Цучиното, Цучиноэ, Хиното, Хиноэ — 2 дня",
    "Киното, Киноэ — 3 дня"
  ],
  paths: [
    {
      label: "Мидзуното → Мидзуноэ",
      items: [
        "Пройди обряд первого наставления."
      ]
    },
    {
      label: "Мидзуноэ → Каното",
      items: [
        "Получи личное поручение от старшего состава и исполни его."
      ]
    },
    {
      label: "Каното → Каноэ",
      items: [
        "Получи личное поручение от старшего состава и исполни его."
      ]
    },
    {
      label: "Каноэ → Цучиното",
      items: [
        "Пройди проверку на знание клятв крови."
      ]
    },
    {
      label: "Цучиното → Цучиноэ",
      items: [
        "Выполни серьёзное поручение от старшего состава."
      ]
    },
    {
      label: "Цучиноэ → Хиното",
      items: [
        "Проведи спарринг с одним из старших членов отряда или наставником. Если членов отряда нет рядом, допускается спарринг с любым подходящим демоном."
      ]
    },
    {
      label: "Хиното → Хиноэ",
      items: [
        "Прими участие минимум в 2-х мероприятиях от Инструктора или старшего состава."
      ]
    },
    {
      label: "Хиноэ → Киното",
      items: [
        "Совместно с инструктором или старшим проведи тренировку. Если провести тренировку невозможно, допускается аналог: убийство 2-х хашир и выпытывание важной информации у пленного."
      ]
    },
    {
      label: "Киното → Киноэ",
      items: [
        "Пройди главное испытание старшего звена Дзюнкоку."
      ]
    }
  ]
});



const siteData = {
  nav: [
    { key: "zenitsu", label: "Зеницу", href: "zenitsu.html" },
    { key: "nezuko", label: "Незуко", href: "nezuko.html" },
    { key: "Inosuke", label: "Иноске", href: "Inosuke.html" },
    { key: "Tazik", label: "Танджиро", href: "Tazik.html" },
    { key: "Kaigaku", label: "Кайгаку", href: "Kaigaku.html" },
    { key: "genia", label: "Генья", href: "genia.html" },
    { key: "Kanao", label: "Канао", href: "Kanao.html" }
  ],
  pages: {
    overview: {
      kicker: "Система повышения",
      title: "Лорные персонажи",
      text: "Система повышения для лорников."
    },
    genia: {
      kicker: "Генья Шинадзугава",
      title: "Система повышения Генья Шинадзугава",
      text: "Порядок повышения по рангам Истребителей."
    },
    Kaigaku: {
      kicker: "Кайгаку",
      title: "Система повышения Кайгаку",
      text: "Порядок повышения по рангам Истребителей."
    },
    Inosuke: {
      kicker: "Иноске Хашибира",
      title: "Система повышения Иноске Хашибира",
      text: "Порядок повышения по рангам Истребителей."
    },
    zenitsu: {
      kicker: "Зеницу Агацума",
      title: "Система повышения Зеницу Агацума",
      text: "Порядок повышения по рангам Истребителей."
    },
    nezuko: {
      kicker: "Незуко Комадо",
      title: "Система повышения Незуко Комадо",
      text: "Порядок повышения по рангам Истребителей."
    },
    Tazik: {
      kicker: "Танджиро Комадо",
      title: "Система повышения Танджиро Комадо",
      text: "Порядок повышения по рангам Истребителей."
    },
    Kanao: {
      kicker: "Канао Цуюри",
      title: "Система повышения Канао Цуюри",
      text: "Порядок повышения по рангам Истребителей."
    }
  },
  overview: {
    intro: [
      "Лорные персонажи:",
    ],
    highlights: [
      {
        title: "Кто они",
        text: "Лорные персонажи."
      },
      {
        title: "Роль",
        text: "Участие в ГМП, продвижение сюжета."
      }
    ]
  },
  promotions: {
    genia: {
  intro: "Порядок повышения по рангам.",
  note: "Если требуется много скриншотов, загружайте их на Imgur или аналогичный сервис. * - задание нельзя пропустить с помощью рекомендации",
  cooldowns: [
    "Мидзуното, Мидзуноэ, Каното, Каноэ — 1 день",
    "Цучиното, Цучиноэ, Хиното, Хиноэ — 2 дня",
    "Киното, Киноэ — 3 дня"
  ],
  paths: [
    {
      label: "Мидзуното → Мидзуноэ",
      items: [
        "Принять участие в тренировке.",
        "Пройти тренировку стрельбы из ничиринового ружья. (мин. 10 действий)*",
        "Пройти тренировку физической подготовки. (мин. 15 действий)*"
      ]
    },
    {
      label: "Мидзуноэ → Каното",
      items: [
        "Привести в порядок ничириновое ружье и вакидзаси. (мин. 20 действий)*",
        "Победить демона в сражении.",
        "Пройти тренировку точности стрельбы. (мин. 15 действий)*",
        "Принять участие в 1 мероприятии"
      ]
    },
    {
      label: "Каното → Каноэ",
      items: [
        "Получить личное поручение от любого Столпа и исполнить его.*",
        "Получить рекомендацию I уровня.",
        "Пройти проверку на знание устава.",
        "Убить 3 демона.",
        "Пройти тренировку по владению вакидзаси. (мин. 15 действий)*",
        "Принять участие в 2 мероприятиях"
      ]
    },
    {
      label: "Каноэ → Цучиното",
      items: [
        "Победить демона в одиночку.",
        "Принять участие в 2 мероприятиях ",
        "Пройти тренировку от любого Столпа.*",
        "Выполнить поручение любого Столпа.*",
        "Выполнить поручение от члена семьи Убуяшики.*"
      ]
    },
    {
      label: "Цучиното → Цучиноэ",
      items: [
        "Пройти силовую тренировку. (мин. 20 действий)*",
        "Получить рекомендацию II уровня.*",
        "Захватить 2 точки",
        "Принять участие в 4 мероприятиях ",
        "Победить 10 демонов."
      ]
    },
    {
      label: "Цучиноэ → Хиното",
      items: [
        "Получить поручение от Столпа Камня и исполнить его.*",
        "Победить 13 демонов.",
        "Пройти тренировку от Любого Столпа.",
        "Пройти тренировку скоростной перезарядки ничиринового ружья. (мин. 15 действий)*",
        "Выполнить поручение от Кагаи Убуяшики.*"
      ]
    },
    {
      label: "Хиното → Хиноэ",
      items: [
        "Принять участие минимум в 2-х мероприятиях.",
        "Пройти 2 тренировки",
        "Пройти тренировку физической подготовки. (мин. 20 действий)*",
        "Победить 15 демонов.",
        "Получить рекомендацию III уровня.",
        "Получить одобрение от Кагаи Убуяшики.*"
      ]
    },
    {
      label: "Хиноэ → Киното",
      items: [
        "Получить признание от 2 Столпов.*",
        "Победить демона, являющегося Луной.*",
        "Добиться признания своих успехов от Тенгена.*",
        "Выполнить три миссии.",
        "Получить рекомендацию от Члена Семьи Убуяшики.*",
        "Получить рекомендацию от Столпа Камня.*"
      ]
    },
    {
      label: "Киното → Киноэ",
      items: [
        "Получить признание от 3 Столпов.*",
        "Победить 2 демона, являющихся Луной.*",
        "Выполнить поручение от Столпа Камня.*",
        "Выполнить поручение от Столпа Ветра.*",
        "Выполнить поручение от Столпа Любви.*",
        "Выполнить поручение от Члена Семьи Убуяшики.*",
        "Принять участие в глобальном мероприятии.",
        "Принять участие в 5 мероприятиях",
        "Убить 20 демонов."
      ]
    }
  ]
},
    Kaigaku: {
  intro: "Порядок повышения по рангам. * - задание нельзя пропустить с помощью рекомендации",
  note: "Если требуется много скриншотов, загружайте их на Imgur или аналогичный сервис.",
  cooldowns: [
    "Мидзуното, Мидзуноэ, Каното, Каноэ — 1 день",
    "Цучиното, Цучиноэ, Хиното, Хиноэ — 2 дня",
    "Киното, Киноэ — 3 дня"
  ],
  paths: [
    {
      label: "Мидзуното → Мидзуноэ",
      items: [
        "Одержать победу над другим истребителем в тренировочном поединке.",
        "Отработать одну из изученных форм Дыхания Грома (мин. 15 действий).*",
        "Пройти тренировку силы ног и рывков."
      ]
    },
    {
      label: "Мидзуноэ → Каното",
      items: [
        "Провести спарринг против Зеницу Агацумы.",
        "Продемонстрировать свое превосходство над Зеницу во время тренировки (мин. 10 действий). *",
        "Убей своего первого Демона"
      ]
    },
    {
      label: "Каното → Каноэ",
      items: [
        "Получить личное поручение от Джигоро и исполнить его.*",
        "Получить рекомендацию I уровня.",
        "Продемонстрировать Джигоро своё владение Дыханием Грома. (мин. 15 действий)*",
        "Пройти проверку на знание устава."
      ]
    },
    {
      label: "Каноэ → Цучиното",
      items: [
        "Победить демона в одиночку.",
        "Провести тренировку владения мечом. (мин. 20 действий)*",
        "Принять участие в МП",
        "Провести спарринг с Зеницу Агацумой.*",
        "Принять участие в тренировке"
      ]
    },
    {
      label: "Цучиното → Цучиноэ",
      items: [
        "Провести тренировку концентрации полного дыхания. (мин. 20 действий)*",
        "Провести спарринг с истребителем, чей ранг не отличается более чем на один.",
        "Получить рекомендацию II уровня.*",
        "Одержать победу над сильным демоном."
      ]
    },
    {
      label: "Цучиноэ → Хиното",
      items: [
      "Выполнить личное поручение Джигоро.*",
      "Победить 7 демонов.",
      "Продемонстрировать превосходное владение Дыханием Грома перед Джигоро.*",
      "Провести 2 тренировки."
      ]
    },
    {
      label: "Хиното → Хиноэ",
      items: [
        "Выполнить поручение любого Столпа.*",
        "Победить 12 демонов.",
        "Пройти проверку на знание устава.*",
        "Провести тренировку для истребителей Дыхания Грома.*",
        "Получить рекомендацию III уровня.*"
      ]
    },
    {
      label: "Хиноэ → Киното",
      items: [
        "Совместно с Джигоро провести тренировку.",
        "Получить признание от двух Столпов.*",
        "Победить демона, являющегося Луной.",
        "Победить 15 демонов.",
        "Принять участие в 4 мероприятиях",
        "Принять участие в 2 мероприятиях от Джигоро.*", 
      ]
    },
    {
      label: "Киното → Киноэ",
      items: [
        "Принять участие в глобальном мероприятии.",
        "Получить признание от трёх Столпов.*",
        "Победить демона, являющегося Луной.*",
        "Выполнить личное поручение Джигоро.*",
        "Выполнить поручение от члена семьи Убуяшики.*"
      ]
    }
  ]
},
    Inosuke: {
  intro: "Порядок повышения по рангам.",
  note: "Если требуется много скриншотов, загружайте их на Imgur или аналогичный сервис. * - задание нельзя пропустить с помощью рекомендации",
  cooldowns: [
    "Мидзуното, Мидзуноэ, Каното, Каноэ — 1 день",
    "Цучиното, Цучиноэ, Хиното, Хиноэ — 2 дня",
    "Киното, Киноэ — 3 дня"
  ],
  paths: [
    {
      label: "Мидзуното → Мидзуноэ",
      items: [
        "Принять участие в тренировке.",
        "Пройти тренировку по выживанию в дикой природе (мин. 10 действий).*",
        "Провести тренировку по охоте на демонов (мин. 15 действий).*"
      ]
    },
    {
      label: "Мидзуноэ → Каното",
      items: [
        "Пройти тренировку по использованию клинков (мин. 10 действий).*",
        "Победить демона в сражении.",
        "Принять участие в мероприятии.",
        "Захватить 5 точек"
      ]
    },
    {
      label: "Каното → Каноэ",
      items: [
        "Убить 5 демонов",
        "Пройти проверку на знание устава.",
        "Получить личное поручение от любого Столпа и исполнить его.*",
        "Провести спарринг с истребителем своего ранга и выше",
        "Участвовать в битве с низшей луной, выжив в ней"
      ]
    },
    {
      label: "Каноэ → Цучиното",
      items: [
        "Устроить дружеское соревнование с Танджиро или Зеницу.",
        "Провести патрулирование территории (мин. 15 действий).",
        "Успешно защитить мирного жителя во время миссии.",
        "Захватить 10 точек"
      ]
    },
    {
      label: "Цучиното → Цучиноэ",
      items: [
        "Принять участие в 3 мероприятиях.",
        "Победить 10 демонов.",
        "Выполнить поручение от любого Столпа.*",
        "Бросить вызов Столпу и получить его расположение.*",
      ]
    },
    {
      label: "Цучиноэ → Хиното",
      items: [
        "Победить 13 демонов.",
        "Принять участие в 4 мероприятиях.",
        "Выполнить поручение от любого Столпа.*",
        "Получить поручения от 2 столпов и исполнить их.*",
        "Получить признание от 2 столпов.*"
      ]
    },
    {
      label: "Хиното → Хиноэ",
      items: [
        "Победить 14 демонов.",
        "Бросить вызов демон-лунe и победить его.*",
        "Принять участие в 4 мероприятиях.",
        "Выполнить поручение от любого Столпа.*",
        "Получить поручения от 3 столпов и исполнить их.*",
        "Бросить вызов как минимум 3 столпам и получить их признание.*"
      ]
    },
    {
      label: "Хиноэ → Киното",
      items: [
        "Получить рекомендацию от члена Семьи Убуяшики.*",
        "Победить 15 демонов.",
        "Принять участие в 2 мероприятиях.",
        "Принять участие в 2 тренировках от Столпов.*",
        "Получить признание от 3 столпов.*",
        "Победить демона, являющегося Луной.*",
        "Выполнить задание от Кагаи Убуяшики.*"
      ]
    },
    {
      label: "Киното → Киноэ",
      items: [
        "Победить 2 демонов, являющихся Луной.",
        "Принять участие в глобальном мероприятии.",
        "Получить признание от 3 столпов.*",
        "Выполнить поручение от Столпа Насекомого.*",
        "Выполнить поручение от Столпа Любви.*",
        "Выполнить поручение от члена Семьи Убуяшики.*"
      ]
    }
  ]
},
    zenitsu: {
  intro: "Порядок повышения по рангам.",
  note: "Если требуется много скриншотов, загружайте их на Imgur или аналогичный сервис. * - задание нельзя пропустить с помощью рекомендации",
  cooldowns: [
    "Мидзуното, Мидзуноэ, Каното, Каноэ — 1 день",
    "Цучиното, Цучиноэ, Хиното, Хиноэ — 2 дня",
    "Киното, Киноэ — 3 дня"
  ],
  paths: [
    {
      label: "Мидзуното → Мидзуноэ",
      items: [
        "Принять участие в тренировке.",
        "Использовать первый стиль дыхания Грома (мин 10 действий).*",
        "Провести патрулирование территории (мин. 15 действий)."
      ]
    },
    {
      label: "Мидзуноэ → Каното",
      items: [
        "Пройти тренировку скорости и реакции.",
        "Заточи катану и приведи её в порядок. (мин. 10 действий)*",
        "Победить в сражении с демоном."
      ]
    },
    {
      label: "Каното → Каноэ",
      items: [
        "Получи личное поручение от Джигоро и исполни его.*",
        "Получи рекомендацию I уровня.",
        "Пройти проверку на знание устава.",
        "Получи поручение от Столпа Грома и исполни его.*"
      ]
    },
    {
      label: "Каноэ → Цучиното",
      items: [
        "Пройти тренировку с другим охотником.",
        "Победить демона в одиночку.",
        "Использовать первый стиль Дыхания Грома несколько раз (мин. 15 действий). *",
        "Провести тренировку (обязательно наличие хотя бы одного истребителя, привязки к рангу нет)"
      ]
    },
    {
      label: "Цучиното → Цучиноэ",
      items: [
        "Провести спарринг с одним из истребителей. (Разница в рангах не должна превышать 1 ранг.)",
        "Провести тренировку выносливости. (мин. 20 действий) (обязательно наличие хотя бы одного истребителя, привязки к рангу и дыханию нет) * ",
        "Получи рекомендацию II уровня. *"
      ]
    },
    {
      label: "Цучиноэ → Хиното",
      items: [
        "Прими участие в тренировке от Джигоро. *",
        "Победи 7 демонов.",
        "Выполни поручение от Столпа Грома или Джигоро. *",
        "Выполни поручение от любого столпа."
      ]
    },
    {
      label: "Хиното → Хиноэ",
      items: [
        "Прими участие минимум в 2-х мероприятиях.",
        "Проведи 2 тренировки с другим охотником. (обязательно наличие хотя бы одного истребителя, привязки к рангу нет)",
        "Проведи тренировку выносливости. (мин. 20 действий) (обязательно наличие хотя бы одного истребителя, привязки к рангу и дыханию нет) *",
        "Победи 10 демонов.",
        "Получи рекомендацию III уровня."
      ]
    },
    {
      label: "Хиноэ → Киното",
      items: [
        "Совместно с Джигоро проведи тренировку.",
        "Получи признание от от 2 столпов*.",
        "Победи 15 демонов.",
        "Победи демона что является Луной.*",
        "Пройди проверку на знание устава.*"
      ]
    },
    {
      label: "Киното → Киноэ",
      items: [
        "Прими участие в глобальном мероприятии.",
        "Получить признание от 3 столпов*.",
        "Победи демона что является Луной.*",
        "Выполни поручение от Столпа Грома или Джигоро.*",
        "Выполни поручение от Столпа Любви или Столпа Звука.*",
        "Выполни поручение от Столпа Ветра или Столпа Камня.*",
        "Выполни поручение от члена Семьи Убуяшики.*"
      ]
    }
  ]
},
    nezuko: promotionTemplate(),
    Tazik: promotionTemplate(),
    Kanao: {
  intro: "Порядок повышения по рангам.",
  note: "Если требуется много скриншотов, загружайте их на Imgur или аналогичный сервис. * - задание нельзя пропустить с помощью рекомендации",
  cooldowns: [
    "Мидзуното, Мидзуноэ, Каното, Каноэ — 1 день",
    "Цучиното, Цучиноэ, Хиното, Хиноэ — 2 дня",
    "Киното, Киноэ — 3 дня"
  ],
  paths: [
    {
  label: "Мидзуното → Мидзуноэ",
  items: [
    "Принять участие в тренировке.",
    "Пройти тренировку по владению Дыханием Цветка. (мин. 15 действий)*",
    "Провести тренировку скорости и точности ударов. (мин. 15 действий)*"
  ]
},
{
  label: "Мидзуноэ → Каното",
  items: [
    "Получить наставление от Шинобу Кочо и выполнить его.*",
    "Победить 3 демонов.",
    "Пройти тренировку уклонений. (мин. 15 действий)*",
    "Принять участие в 1 мероприятии."
  ]
},
{
  label: "Каното → Каноэ",
  items: [
    "Получить личное поручение от Столпа Насекомого и исполнить его.*",
    "Получить рекомендацию I уровня.",
    "Пройти проверку на знание устава.",
    "Продемонстрировать одну из форм Дыхания Цветка. (мин. 15 действий)*",
    "Победить 3 демонов."
  ]
},
{
  label: "Каноэ → Цучиното",
  items: [
    "Победить демона в одиночку.",
    "Принять участие в 2 мероприятиях.",
    "Пройти тренировку реакции и концентрации. (мин. 20 действий)*",
    "Выполнить поручение от Столпа Насекомого.*",
  ]
},
{
  label: "Цучиното → Цучиноэ",
  items: [
    "Получить рекомендацию II уровня.*",
    "Победить 5 демонов.",
    "Пройти тренировку на развитие остроты зрения. (мин. 20 действий)*",
    "Принять участие в 4 мероприятиях.",
    "Оказать помощь раненому союзнику."
  ]
},
{
  label: "Цучиноэ → Хиното",
  items: [
    "Получить поручение от Шинобу Кочо и исполнить его.*",
    "Победить 7 демонов.",
    "Пройти тренировку от любого Столпа.*",
    "Провести тренировку Дыхания Цветка. (мин. 20 действий)*",
    "Выполнить поручение от Кагаи Убуяшики.*"
  ]
},
{
  label: "Хиното → Хиноэ",
  items: [
    "Принять участие минимум в 2 мероприятиях.",
    "Пройти 2 тренировки.",
    "Провести тренировку на развитие скорости. (мин. 20 действий)*",
    "Победить 8 демонов.",
    "Получить рекомендацию III уровня."
  ]
},
{
  label: "Хиноэ → Киното",
  items: [
    "Получить признание от 2 Столпов.*",
    "Победить демона, являющегося Луной.*",
    "Получить одобрение Шинобу Кочо.*",
    "Выполнить задание от члена Семьи Убуяшики.",
    "Получить рекомендацию от члена Семьи Убуяшики.*"
  ]
},
{
  label: "Киното → Киноэ",
  items: [
    "Получить признание от 3 Столпов.*",
    "Победить 2 демонов, являющихся Луной.*",
    "Выполнить поручение от Столпа Насекомого.*",
    "Выполнить поручение от члена семьи Убуяшики.*",
    "Принять участие в глобальном мероприятии.",
    "Принять участие в 5 мероприятиях.",
    "Победить 10 демонов."
  ]
}
  ]
}
  }
};

const pageKey = document.body.dataset.page || "overview";
const quickNav = document.getElementById("quick-nav");
const heroKicker = document.getElementById("hero-kicker");
const heroTitle = document.getElementById("hero-title");
const heroText = document.getElementById("hero-text");
const app = document.getElementById("app");

function renderTopbar() {
  quickNav.innerHTML = siteData.nav
    .map((item) => `<a class="topbar-link ${item.key === pageKey ? "is-active" : ""}" href="${item.href}">${item.label}</a>`)
    .join("");
}

function renderHero() {
  const page = siteData.pages[pageKey] || siteData.pages.overview;
  heroKicker.textContent = page.kicker;
  heroTitle.textContent = page.title;
  heroText.textContent = page.text;
  document.title = `Система повышения | ${page.title}`;
}

function renderIndexedList(items) {
  return items.map((item) => `<li>${item}</li>`).join("");
}

function renderOverviewPage() {
  app.innerHTML = `
    <section class="panel">
      <div class="section-heading">
        <div>
          <p class="section-kicker">Общая информация</p>
          <h2>Общая информация</h2>
        </div>
      </div>
      <article class="spotlight-card">
        ${siteData.overview.intro.map((text) => `<p class="page-copy">${text}</p>`).join("")}
        <div class="hero-actions">
          ${siteData.nav.map((item) => `<a class="hero-button" href="${item.href}">${item.label}</a>`).join("")}
        </div>
      </article>
    </section>
    <section class="panel">
      <div class="cards-grid">
        ${siteData.overview.highlights
          .map(
            (item, index) => `
              <article class="info-card">
                <div class="number-badge">${index + 1}</div>
                <h3>${item.title}</h3>
                <p class="page-copy">${item.text}</p>
              </article>
            `
          )
          .join("")}
      </div>
    </section>
  `;
}

function renderPromotionPage() {
  const promotion = siteData.promotions[pageKey];

  if (!promotion) {
    app.innerHTML = "";
    return;
  }

  app.innerHTML = `
    <section class="panel">
      <div class="section-heading">
        <div>
          <p class="section-kicker">Повышение</p>
          <h2>Повышение по ступеням</h2>
        </div>
      </div>
      <article class="spotlight-card">
        <p class="page-copy">${promotion.intro}</p>
      </article>
    </section>
    <section class="panel">
      <div class="promotion-tabs" id="promotion-tabs">
        ${promotion.paths
          .map(
            (path, index) => `
              <button class="tab-button ${index === 0 ? "is-active" : ""}" type="button" data-index="${index}">
                ${path.label}
              </button>
            `
          )
          .join("")}
      </div>
      <div id="promotion-panes">
        ${promotion.paths
          .map(
            (path, index) => `
              <section class="promotion-pane ${index === 0 ? "is-active" : ""}" data-index="${index}">
                <article class="step-card">
                  <div class="step-card__title">
                    <div class="index-badge">${index + 1}</div>
                    <div>
                      <h4>${path.label}</h4>
                      ${path.subtitle ? `<div class="rank-card__meta">${path.subtitle}</div>` : ""}
                    </div>
                  </div>
                  <ol class="ordered-list">
                    ${renderIndexedList(path.items)}
                  </ol>
                </article>
              </section>
            `
          )
          .join("")}
      </div>
      <div class="inline-note">${promotion.note}</div>
    </section>
    <section class="panel">
      <div class="section-heading">
        <div>
          <p class="section-kicker">КД</p>
          <h2>КД на повышение</h2>
        </div>
      </div>
      <div class="cooldown-grid">
        ${promotion.cooldowns
          .map(
            (item) => `
              <article class="simple-card">
                <p>${item}</p>
              </article>
            `
          )
          .join("")}
      </div>
    </section>
  `;

  const buttons = document.querySelectorAll("#promotion-tabs .tab-button");
  const panes = document.querySelectorAll(".promotion-pane");

  buttons.forEach((button) => {
    button.addEventListener("click", () => {
      const index = button.dataset.index;

      buttons.forEach((item) => {
        item.classList.toggle("is-active", item.dataset.index === index);
      });

      panes.forEach((pane) => {
        pane.classList.toggle("is-active", pane.dataset.index === index);
      });
    });
  });
}

function renderPage() {
  if (pageKey === "overview") {
    renderOverviewPage();
  } else {
    renderPromotionPage();
  }
}

renderTopbar();
renderHero();
renderPage();
